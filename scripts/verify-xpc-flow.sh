...
usage() {
  cat <<'USAGE'
Usage: verify-xpc-flow.sh [--shot <path>] [--display-name <name>] [--display-id <id>] [--out <path>]
...
if [[ -z "$shot_bin" ]]; then
  # In development, prefer the latest Debug build in DerivedData.
...
if ! grep -q '"endpoint" : "xpc"' "$tmp_doctor"; then
  echo "doctor output is not endpoint=xpc; verify you are using the correct ShotCli build." >&2
  exit 10
fi
...
if [[ $rc_capture -eq 11 ]]; then
  echo "capture blocked by missing Screen Recording permission (expected before granting)." >&2
fi
